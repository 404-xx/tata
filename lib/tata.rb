...
module Tata
  ...
  class Article < Hash
    ...
    def load
      data = if @obj.is_a? String
        ...
        # use the date from the filename, or else tata won't find the article
        @obj =~ /\/(\d{4}-\d{2}-\d{2})[^\/]*$/
  ...
  class Config < Hash
    Defaults = {
      ...
      :error => lambda {|code|                              # The HTML for your error page
        "<font style='font-size:300%'>Wow! We are living on Mars now(#{code})</font>"
      }
    }
    def initialize obj
  ...
  class Server
    ...
    def call env
      ...
      @response['Content-Length'] = (response_body.respond_to?(:bytesize) ? response_body.bytesize : response_body.size).to_s unless response_body.empty?
      @response['Content-Type']   = Rack::Mime.mime_type(".#{response[:type]}")

      # Set http cache headers
      @response['Cache-Control'] = if Tata.env == 'production'
